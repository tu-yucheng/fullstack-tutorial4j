...
class HiddenClassUnitTest {
	...
	void whenInvokeMethodOnHiddenClass_thenSuccess() {
		...
		try {
			...
			assertNull(hiddenClass.getCanonicalName());

			assertThrows(ClassNotFoundException.class, () -> Class.forName(hiddenClass.getName()));

			assertThrows(ClassNotFoundException.class, () -> lookup.findClass(hiddenClass.getName()));
