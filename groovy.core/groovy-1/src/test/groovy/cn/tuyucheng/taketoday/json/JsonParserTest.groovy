...
class JsonParserTest extends Specification {
    ...
    def 'Should parse to Account given Json String with date property'() {
        given:
        def json = '{"id":"1234","value":15.6,"createdAt":"2017-12-31T16:00:00+0000"}'
        when:
    ...
    def 'Should parse to Json given on Account object'() {
        given:
        Account account = new Account(
                id: '123',
                value: 15.6,
                createdAt: new SimpleDateFormat('yyyy-MM-dd').parse('2018-01-01')
        )
        when:
        def json = jsonParser.toJson(account)
        then:
        json == '{"value":15.6,"createdAt":"2017-12-31T16:00:00+0000","id":"123"}'
    }
