...
public class CalculatorUnitTest {
...

	{
		Suite.describe("Lambda behave example tests", it -> {
			...
			it.isSetupWith(() -> calculator = new Calculator(1, 2));
			it.should("Add the given numbers", expect -> expect.that(calculator.add()).is(3));
			it.should("Throw an exception if divide by 0", expect -> expect.exception(ArithmeticException.class, () -> calculator.divide(1, 0)));
			it.uses(2, 3, 5)
					.and(23, 10, 33)
					.toShow("%d + %d = %d", (expect, a, b, c) -> expect.that(calculator.add(a, b)).is(c));
			it.requires(2)
