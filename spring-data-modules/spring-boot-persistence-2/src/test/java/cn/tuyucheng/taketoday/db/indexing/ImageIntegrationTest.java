...
class ImageIntegrationTest {
	...
	private MockMultipartHttpServletRequestBuilder createUploadImageRequest() throws IOException {
		...
		return MockMvcRequestBuilders.multipart("/image")
				.file(new MockMultipartFile("multipartImage", "tuyucheng", MediaType.TEXT_PLAIN_VALUE, image));
	}
	...
	private Image tuyuchengImage() throws IOException {
		...
		mockImage.setContent(Files.readAllBytes(Paths.get(classLoader.getResource("tuyucheng.jpeg")
				.getFile())));
		return mockImage;
	}
}