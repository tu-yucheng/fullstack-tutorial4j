...
class FileSystemImageIntegrationTest {
	...
	void givenJpegImage_whenUploadIt_thenReturnItsId() throws Exception {
		...
		InputStream image = classLoader.getResourceAsStream("tuyucheng.jpeg");

		MockMultipartFile jpegImage = new MockMultipartFile("image", "tuyucheng", MediaType.TEXT_PLAIN_VALUE, image);
		MockMultipartHttpServletRequestBuilder multipartRequest = MockMvcRequestBuilders.multipart("/file-system/image")
				.file(jpegImage);

		given(fileLocationService.save(jpegImage.getBytes(), "tuyucheng"))
				.willReturn(1L);
	...
	private FileSystemResource tuyuchengJpegResource() {
		ClassLoader classLoader = ClassLoader.getSystemClassLoader();
		String imagePath = classLoader.getResource("tuyucheng.jpeg").getFile();
